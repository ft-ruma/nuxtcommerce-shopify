...
export const CHECKOUT_CART_KEY = 'shopify-checkout-cart-id';
...
export const useCart = () => {
  ...
  const handleAddToCart = async (productId: number) => {
    ...
    try {
      ...
      const response = await $fetch<AddToCartResponse>('/api/cart/add', {
        method: 'POST',
        body: { productId, quantity: (existing?.quantity ?? 0) + 1 },
      });
  ...
  // After a shopper returns from Shopify checkout, clear the cart if the order was placed.
  const checkedCheckout = useState<boolean>('checkedCheckout', () => false);
  const syncCompletedCheckout = async () => {
    if (checkedCheckout.value) return;
    checkedCheckout.value = true;
    const cartId = localStorage.getItem(CHECKOUT_CART_KEY);
    if (!cartId) return;

    try {
      const { completed } = await $fetch<{ completed: boolean }>('/api/cart/status', { query: { id: cartId } });
      if (completed) {
        localStorage.removeItem(CHECKOUT_CART_KEY);
        setCart([]);
      }
    } catch {
      // Network or config issue: keep the cart as it is.
    }
  };
